...
The goal of running the autocorrelation function (ACF) for squared returns is to detect volatility clustering.
Significance in autocorrelations in squared returns indicate that large changes
tend to be followed by large changes (of either sign) and small changes tend to be followed by small changes,
which is a key characteristic of GARCH models.
...
Now we'll fit a GARCH(1,1) model to the returns and retrieve values of the coefficients with
...
Section examines the autocorrelation function (ACF) of squared standardized residuals from the model.
ACF helps us check if our model has successfully captured volatility clustering in the data.

If the model is well-specified, these squared standardized residuals should show no significant autocorrelation,
indicating that no predictable volatility patterns remain unexplained by the model.
